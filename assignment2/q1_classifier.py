...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        #损失函数
        with tf.variable_scope("transformation"):#定义一个命名空间，没啥好说的
            #定义一个bias，注意维度是类别数
            bias = tf.Variable(tf.random_uniform([self.config.n_classes]))
            #然后是W，输入是64x100，那么W就是100x5，特征x分类数
